    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <nav class="mb-6">
            <ol class="flex items-center space-x-2 text-sm px-4 py-3 border border-purple-100">
                ...
                <li class="flex items-center text-gray-400">
                    <i class="fas fa-chevron-right text-xs mx-2"></i>
                    <span class="text-gray-700 font-semibold">Editar Rol</span>
                </li>
            </ol>
        </nav>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-purple-100 p-6 mb-6">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-gray-800 mb-1">Editar Rol</h1>
                    <p class="text-gray-600 text-sm">Actualice la información del rol: {{ $rol->tipo }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-purple-100 p-6">
            <form action="{{ route('roles.update', $rol->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                        <div class="flex items-center space-x-2 mb-2">
                            ...
                            <span class="text-sm font-medium text-gray-600">ID del Rol</span>
                        </div>
                        <p class="text-lg font-semibold text-gray-800">{{ $rol->id }}</p>
                    </div>
                ...
                <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                    <div class="flex items-start space-x-3">
                        ...
                        <div>
                            <h4 class="text-sm font-medium text-yellow-800 mb-1">Consideraciones importantes</h4>
                            <ul class="text-xs text-yellow-700 space-y-1">
                                <li class="flex items-start space-x-2">
                                    <i class="fas fa-circle text-yellow-400 mt-0.5 text-xs"></i>
                                    <span>Los cambios en el nombre del rol pueden afectar los permisos del sistema</span>
                                </li>
                                <li class="flex items-start space-x-2">
                                    <i class="fas fa-circle text-yellow-400 mt-0.5 text-xs"></i>
                                    <span>Verifique que el nuevo nombre no entre en conflicto con otros roles</span>
                                </li>
                                <li class="flex items-start space-x-2">
                                    <i class="fas fa-circle text-yellow-400 mt-0.5 text-xs"></i>
                                    <span>Los usuarios asignados a este rol se actualizarán automáticamente</span>
                                </li>
                            </ul>
                        </div>
                    </div>
        ...
        <div class="mt-6 bg-white rounded-xl shadow-lg border border-purple-100 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-600">
                <div>
                    <p class="font-medium text-gray-700 mb-2">Usuarios con este rol:</p>
                    <p class="text-purple-600 font-semibold">
                        {{ isset($rol->users_count) ? $rol->users_count : ($rol->users ? $rol->users->count() : 0) }} usuarios
                    </p>
                </div>
                <div>
                    <p class="font-medium text-gray-700 mb-2">Última actualización:</p>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Resaltar cambios si el valor es diferente al original
        const originalValue = "{{ $rol->tipo }}";
        if (tipoInput && tipoInput.value !== originalValue) {
            tipoInput.classList.add('border-purple-300', 'bg-purple-50');
        }
    });
</script>
@endsection